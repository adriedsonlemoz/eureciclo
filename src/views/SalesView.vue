<template>
  <div class="min-h-screen pt-safe">
    ...
    <div v-if="!sales.length" class="flex flex-col items-center justify-center px-5 py-20 text-center">
      <div class="w-20 h-20 rounded-3xl flex items-center justify-center mb-5 text-eco-500" style="background:#f0fdf4;border:2px dashed #d1fae5;"><svg class="w-9 h-9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3h12v18l-3-2-3 2-3-2-3 2V3Z"/><path d="M9 8h6M9 12h6"/></svg></div>
        ...
        <div v-for="sale in sales" :key="sale.id" class="glass-card rounded-2xl overflow-hidden">
          <div class="px-4 pt-4 pb-3 flex items-start justify-between gap-2">
            ...
            <div class="flex items-center gap-2 shrink-0">
              ...
              <button @click="deleteTarget = sale" class="w-10 h-10 rounded-xl flex items-center justify-center active:scale-90" style="background:#fff1f2;border:1px solid #fecdd3;" aria-label="Excluir venda">
            ...
            <div v-if="expandedIds.has(sale.id)">
              ...
              <div class="px-4 py-3 flex flex-col gap-2">
                <div v-for="item in sale.items" :key="`${sale.id}-${item.materialId}`" class="flex items-center gap-3 py-1.5 rounded-xl px-2" style="background:#f8fafc;">
                  <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-slate-500" style="background:#eefbf2;"><MaterialIcon :name="item.name" :category="item.category" :legacy-icon="item.icon" class="w-4 h-4" /></div>
          ...
          <div v-if="!expandedIds.has(sale.id)" class="px-4 pb-3 flex gap-1.5 items-center">
            <div v-for="item in sale.items.slice(0, 6)" :key="`${sale.id}-icon-${item.materialId}`" class="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500" style="background:#f0fdf4;">
              <MaterialIcon :name="item.name" :category="item.category" :legacy-icon="item.icon" class="w-3.5 h-3.5" />
            </div>
    ...
    <transition name="modal">
      <div v-if="confirmClear" class="fixed inset-0 z-50 flex items-end justify-center px-5 modal-safe-bottom" style="background:rgba(0,0,0,.4);backdrop-filter:blur(4px);" @click.self="confirmClear = false">
        <div class="w-full bg-white rounded-3xl p-5 max-w-sm">
